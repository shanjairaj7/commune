...
// EAS on Base mainnet (pre-deployed by Ethereum Foundation)
const EAS_CONTRACT = '0x4200000000000000000000000000000000000021';
const SCHEMA_REGISTRY = '0x4200000000000000000000000000000000000020';

// Schema string — registered once via registerReputationSchema.ts script
...
// Set after schema registration — stored in env
...
export async function aggregateWalletMetrics(orgId: string): Promise<ReputationMetrics | null> {
  ...
  const [deliveryStats, uniqueContactCount, inboundCount] = await Promise.all([
    // 1. Delivery metrics: count by status for outbound messages
    messages.aggregate([
      { $match: { 'metadata.domain_id': { $exists: true }, direction: 'outbound', org_id: orgId } },
      {
        $group: {
          _id: null,
          sent: { $sum: 1 },
    ...
    // 2. Unique contacts: distinct recipient addresses
    messages.aggregate([
    ...
    // 3. Inbound message count (for reply rate)
  ...
  const divisor = sent || 1; // avoid division by zero
...
export async function publishAttestation(
  ...
  try {
    // Lazy-load EAS SDK (heavy dependency)
 ...
 * For on-chain verification, consumers read EAS directly.
 */
